...
std::optional<SimpleBLE::ByteArray> SimpleBLE::Safe::Peripheral::readBytes(BluetoothUUID const& service,
                                                                           BluetoothUUID const& characteristic,
                                                                           BluetoothUUID const& descriptor) noexcept {
    try {
        return SimpleBLE::Peripheral::readBytes(service, characteristic, descriptor);
    } catch (...) {
        return std::nullopt;
    }
}
...
bool SimpleBLE::Safe::Peripheral::write(BluetoothUUID const& service, BluetoothUUID const& characteristic,
                                        BluetoothUUID const& descriptor, ByteStrArray const& data) noexcept {
    try {
